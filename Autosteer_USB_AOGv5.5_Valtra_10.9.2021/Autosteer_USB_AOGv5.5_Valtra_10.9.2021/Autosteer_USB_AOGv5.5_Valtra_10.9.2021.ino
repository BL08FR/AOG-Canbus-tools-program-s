...
//------------------------------------
//Valtra CAN-BUS AOGv5.5 - (Tony's Version)
...
uint16_t setCurve = 32000;       //Variable for Set Curve to S-Bus
uint16_t estCurve = 32000;       //Variable for WAS from S-Bus
int16_t setCurveDiff = 0;
uint16_t lastSetCurve = 32000;
...
digitalWrite(engageLED,LOW);
 ...
 CAN0.init_Mask(0,1,0x00FFFFFF);       // Init first mask...  
  CAN0.init_Filt(0,1,0x0CAC1C13);       // Init first filter...  
  CAN0.init_Filt(1,1,0x0CAC1C13);       // Init second filter... 
...
CAN0.sendMsgBuf(0x18EEFF1C, 1, 8, data1);   //Claim Address
...
//--------CAN-End------  
    ...
    if (Serial.available() > dataLength && isHeaderFound && isPGNFound)
    {
      ...
      {       
        ...
        steerConfig.IsDanfoss = Serial.read(); //uint8_t 8
              
        Serial.read(); //uint8_t 9
        Serial.read(); //uint8_t 10
         
        Serial.read(); //uint8_t 11
        Serial.read(); //uint8_t 12
